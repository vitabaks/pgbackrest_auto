...
`pgbackrest_testrestore.sh` runs **additional advanced checks on your data** after the recovery stage. In case of PostgreSQL data restore and recovery, the stage of physical data validation is performed (all data blocks are sequentially read with pg_dump);
...
![pgbackrest_testrestore.sh_scheme](./scheme.png)

#### Example
```
pgbackrest_testrestore.sh --from=app-db --to=/bkpdata/10/app-db --backup-host=10.128.50.50 --pgver=10 --checkdb --clear --report
```
###### script output:
```
...
2019-06-17 15:57:50 INFO: Starting. Run settings: Log: /var/log/pgbackrest/pgbackrest_testrestore.sh-restore_app-db.log
2019-06-17 15:57:50 INFO: Starting. Run settings: Lock run: /tmp/pgbackrest_testrestore.sh_app-db.lock
...
#### pgbackrest_testrestore.sh --help
```
/usr/bin/pgbackrest_testrestore.sh
...
Usage: /usr/bin/pgbackrest_testrestore.sh --from=STANZANAME --to=DATA_DIRECTORY [ --datname=DATABASE [...] ] [ --recovery-type=( default | immediate | time ) ] [ --recovery-target=TIMELINE  [ --backup-set=SET ] [ --backup-host=HOST ] [ --pgver=( 94 | 10 ) ] [ --checkdb ] [ --clear ] [ --report ] ]
...
--report
        Send report to mail address (Specify smtp parameters in the /usr/bin/pgbackrest_testrestore.sh file)
...
| Restore last backup.
    ...
    # /usr/bin/pgbackrest_testrestore.sh --from=app-db --to=/var/lib/postgresql/11/rst

| Restore latest backup (recover to the end of the archive stream).

    # /usr/bin/pgbackrest_testrestore.sh --from=app-db --to=/var/lib/postgresql/11/rst --recovery-type=default

| Restore backup made a few days ago.

    # /usr/bin/pgbackrest_testrestore.sh --from=app-db --to=/var/lib/postgresql/11/rst --backup-set=20180807-212125F_20180808-050002I

| Restore backup made a few days ago and pick time.

    # /usr/bin/pgbackrest_testrestore.sh --from=app-db --to=/var/lib/postgresql/11/rst --backup-set=20180807-212125F_20180808-050002I --recovery-type=time --recovery-target="2018-08-08 12:46:54"

| Restore backup made a few days ago and pick time. And we have restore only one database with the name "app_db".

    # /usr/bin/pgbackrest_testrestore.sh --from=app-db --to=/var/lib/postgresql/11/rst --datname=app_db --backup-set=20180807-212125F_20180808-050002I --recovery-type=time --recovery-target="2018-08-08 12:46:54"

| Restore and Validate of databases (for example: pgBacRest repository 10.128.64.50, PostgreSQL version 11)

    # /usr/bin/pgbackrest_testrestore.sh --from=app-db --to=/var/lib/postgresql/11/rst --backup-host=10.128.64.50 --pgver=11 --checkdb
...
01 00 * * 6 if pgbackrest --stanza=app-db --type=full backup; then pgbackrest_testrestore.sh --from=app-db --to=/bkpdata/10/app-db --backup-host=10.128.50.50 --checkdb --clear --report; fi
01 00 * * 0-5 if pgbackrest --stanza=app-db --type=diff backup; then pgbackrest_testrestore.sh --from=app-db --to=/bkpdata/10/app-db --backup-host=10.128.50.50 --checkdb --clear --report; fi

30 00 * * 6 if pgbackrest --stanza=apdb-cluster --type=full backup; then pgbackrest_testrestore.sh --from=apdb-cluster --to=/bkpdata/11/apdb-cluster --backup-host=10.128.50.50 --pgver=11 --checkdb --clear --report; fi
30 00 * * 0-5 if pgbackrest --stanza=apdb-cluster --type=diff backup; then pgbackrest_testrestore.sh --from=apdb-cluster --to=/bkpdata/11/apdb-cluster --backup-host=10.128.50.50 --pgver=11 --checkdb --clear --report; fi

00 01 * * 6 if pgbackrest --stanza=dbs-eu--type=full backup; then pgbackrest_testrestore.sh --from=dbs-eu--to=/bkpdata/9.4/dbs-eu--backup-host=10.128.50.50 --pgver=94 --checkdb --clear --report; fi
00 01 * * 0-5 if pgbackrest --stanza=dbs-eu--type=diff backup; then pgbackrest_testrestore.sh --from=dbs-eu--to=/bkpdata/9.4/dbs-eu--backup-host=10.128.50.50 --pgver=94 --checkdb --clear --report; fi
...
- Specify smtp parameters `smtp_server`, `mail_from`, `mail_to` in the `/usr/bin/pgbackrest_testrestore.sh` file
...
## Installation
1. Download and copy the `pgbackrest_testrestore.sh` script to `/usr/bin/` directory
...
Are [welcome](https://github.com/vitabaks/pgbackrest_testrestore.sh/issues)!
